...
if uploaded_file is not None:
  ...
  try:
    ...
    st.write("出欠：0（欠場）、1（出場）")
    st.write("打撃力：その選手の打撃力の能力を数値化")
    st.write("守指定（その選手に守らせたいポジション）：0(指定なし)、1~9（ピッチャー～ライトの守備番号）")
  ...
  #1回目のファイル読み込み
      ...
      for i in range(1,reg):
        for ii in range(i+1,reg+1):
          for t in T:
            if (i,ii) in Y:
              problem += Y[(i,ii)] >= lpSum(x[(i,j,t)] + x[(ii,j,t)] for j in J if (i,j,t) in x if (ii,j,t) in x) - 1, f"constraint_prior_same_team_{i}_{ii}_{t}"
